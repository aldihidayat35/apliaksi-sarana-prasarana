    ...
    <div class="header-brand">
        <a href="{{ route('admin.dashboard') }}" class="d-flex align-items-center gap-2 text-decoration-none">
            @if(app_setting('app_logo'))
                <img alt="Logo" src="{{ asset('storage/' . app_setting('app_logo')) }}" class="h-30px"/>
            @else
                <i class="ki-duotone ki-abstract-26 fs-2 text-white opacity-75"><span class="path1"></span><span class="path2"></span></i>
            @endif
            <span class="text-white fw-bolder fs-5 text-nowrap aside-name">{{ app_setting('app_name', config('app.name')) }}</span>
        </a>
